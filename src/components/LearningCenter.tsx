...
export default function LearningCenter({
  ...
  const getInitialCourse = (): Course => {
    ...
    if (activeCourseId) {
      const activeFound = courses.find(
  ...
  const [activeCourse, setActiveCourse] =
    useState<Course>(initialCourse);

  const [selectedLesson, setSelectedLesson] = useState<Lesson>(
    initialCourse.lessons?.[0] || emptyLesson
  );
  ...
  const [quizSuccess, setQuizSuccess] =
    useState(false);
  ...
  if (isBlocked) {
    return (
      <div className="py-20 px-4 max-w-3xl mx-auto text-center rtl-dir">
        <div className="bg-[#0f172a] border border-rose-500/30 rounded-2xl p-10">
          <Lock className="w-12 h-12 text-rose-400 mx-auto mb-5" />
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            تم إيقاف صلاحية الوصول إلى المحتوى التعليمي.
            يرجى التواصل مع إدارة الأكاديمية.
          </p>
        </div>
      </div>
    );
  ...
  /*
   * ============================================================
   * SYNC COURSE WHEN COURSES CHANGE
   * ============================================================
   */

  useEffect(() => {
    if (!courses || courses.length === 0) {
      setActiveCourse(emptyCourse);
    ...
    if (activeCourse.id) {
      updatedCourse = courses.find(
        (course) => course.id === activeCourse.id
      );
    }

    if (!updatedCourse && activeCourseId) {
      updatedCourse = courses.find(
        (course) => course.id === activeCourseId
      );
    }

    if (!updatedCourse && initialCourseId) {
      updatedCourse = courses.find(
        (course) => course.id === initialCourseId
      );
    }
    ...
    if (!updatedCourse) {
      updatedCourse = courses[0];
    }

    setActiveCourse(updatedCourse);

    const updatedLesson =
      updatedCourse.lessons?.find(
        (lesson) => lesson.id === selectedLesson.id
      ) ||
      updatedCourse.lessons?.[0] ||
      emptyLesson;

    setSelectedLesson(updatedLesson);
  }, [courses, activeCourseId, initialCourseId]);
  ...
  const allowedLessons5Pct = Math.max(
    1,
    Math.ceil(
      (activeCourse.lessons?.length || 1) * 0.05
    )
  );
  ...
  const isLessonRestricted5Pct =
    !hasFullAccess &&
    currentLessonIndex >= allowedLessons5Pct &&
   ...
   *
   * FIX:
   * isTeacherAdmin was undefined in the original code.
   *
   * Full access now acts as admin/teacher access.
   * ============================================================
   */

  const prevLessonObj =
    currentLessonIndex > 0
      ? activeCourse.lessons[currentLessonIndex - 1]
      : null;

  const isLessonSequentiallyLocked =
    !hasFullAccess &&
    currentLessonIndex > 0 &&
    !!prevLessonObj &&
    !completedLessons.includes(prevLessonObj.id);
  ...
  const handleLessonChange = (
    ...
    if (!hasFullAccess) {
      if (index >= allowedLessons5Pct) {
        return;
      }

      if (index > 0) {
        const previousLesson =
          activeCourse.lessons[index - 1];
  ...
  const handleQuizSubmit = () => {
    ...
    const isCorrect =
      selectedOption ===
      selectedLesson.quiz.correctIndex;

    setQuizSuccess(isCorrect);

    if (isCorrect) {
      const lessonKey =
        `quiz-${selectedLesson.id}`;
   ...
   * VIDEO COMPLETION
   *
   * YouTube iframe cannot reliably tell React that the video
   * reached 100% without the YouTube IFrame API.
   *
   * For now, full-access users can manually confirm watching,
   * while restricted users remain protected by the 5% rule.
   * ============================================================
   */

  const handleMarkVideoCompleted = () => {
  ...
  const handleCompleteLesson = () => {
    ...
    if (!videoCompleted) {
      setLessonCompletionError(
        "يجب مشاهدة المحاضرة بالكامل قبل فتح المحاضرة التالية."
      );
      return;
    }
    ...
    if (
      ...
      setLessonCompletionError(
        "يجب اجتياز الاختبار أولاً لفتح الدرس التالي."
      );
      return;
    }
      ...
      try {
        await fetch(
          "/api/inquiries",
          {
            ...
            body: JSON.stringify({
              title:
                `طلب ترخيص المحتوى الكامل للدورة: ${activeCourse.titleAr}`,
              ...
              content:
                `طلب إذن ترقية من 5% إلى 100% للمشاهدة والمتابعة الكاملة. ملاحظات الطالب: ${
                  requestNotes ||
                  "أرجو منحي صلاحية إكمال باقي الدورة."
                }`,
            }),
          }
        );
  ...
  if (!courses || courses.length === 0) {
    return (
      <div className="py-20 px-4 max-w-4xl mx-auto rtl-dir">
        <div className="bg-[#0f172a] border border-white/5 rounded-2xl p-12 text-center shadow-xl">
          <Layers className="w-14 h-14 text-indigo-400 mx-auto mb-5" />
          ...
          <p className="text-sm text-slate-400">
            لم يتم إضافة أي محتوى تعليمي حتى الآن.
          </p>
        </div>
      </div>
    );
    ...
    <div className="py-12 px-4 sm:px-6 max-w-7xl mx-auto rtl-dir">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 px-3.5 py-1.5 rounded-full text-indigo-300 text-xs font-semibold mb-3">
          ...
          <span>
            منصة تعليمية مشفرة ومحمية | مشغل الفيديو الآمن
          </span>
        </div>

        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">
          مركز المحاضرات والتعلم
        </h2>
      ...
      {hasFullAccess ? (
        <div className="mb-6 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center justify-between gap-3 text-xs text-emerald-300">
          <div className="flex items-center gap-2 font-bold">
            ...
            <span>
              حالة تصريح حسابك: صلاحية كاملة (100%) - يمكنك حضور ومشاهدة جميع المحاضرات واجتياز الاختبارات
            </span>
          </div>
        ...
        <div className="mb-6 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-between gap-3 text-xs text-amber-300">
          <div className="flex items-center gap-2 font-bold">
            ...
            <span>
              حالة تصريح حسابك: عينة 5% (المحاضرة التمهيدية فقط) - تحتاج إلى تصريح 100% من المشرف لمتابعة باقي الدروس
            </span>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-right p-3.5 rounded-xl transition-all duration-200 flex items-center justify-between gap-3 border ${
                      isSelected
                        ...
                        : isLocked
                        ? "bg-[#131b2e]/20 border-transparent text-slate-500"
                  ...
                  >
                    <div className="space-y-1 overflow-hidden">
                      ...
                      <p className="text-[10px] text-slate-500 font-mono ltr-dir">
                        {lesson.duration}
                      </p>
                    </div>
                    ...
                    {lessonDone ? (
                      ...
                      <div
                        className="p-1 rounded bg-amber-500/10 text-amber-400 shrink-0"
                        title="يتطلب تصريح 100% من المشرف"
                      >
                        <Lock className="w-3.5 h-3.5" />
                      </div>
                    ) : isSequentialLocked ? (
                      <div
                        className="p-1 rounded bg-slate-800/80 text-slate-400 shrink-0"
                        title="يجب إكمال المحاضرة السابقة أولاً"
                      >
                        <Lock className="w-3.5 h-3.5" />
                      </div>
                    ) : (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {isLessonRestricted5Pct ? (
            <div className="bg-[#0f172a] border-2 border-indigo-500/30 rounded-2xl p-8 sm:p-12 text-center space-y-6 shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                <h3 className="text-xl font-bold text-white">
                  يتطلب هذا الدرس تصريحاً من المشرف العام
                </h3>

                <p className="text-xs sm:text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
                  بموجب سياسة الأكاديمية، حسابك حالياً مصرح بنسبة 5% فقط (المحاضرة التمهيدية الأولى). لإكمال باقي الكورس ومشاهدة هذا الدرس، يرجى طلب تصريح من المشرف العام{" "}
                  <span className="text-indigo-400 font-bold">
                    المهندس عبدالوهاب أحمد
                  </span>{" "}
                  لتفعيل الصلاحية الكاملة (100%) لحسابك.
                </p>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>
                    طلب فتح الكورس بالكامل (100%) من الأدمن
                  </span>
                </button>
              </div>
            </div>
            ...
            <div className="bg-[#0f172a] border border-amber-500/30 rounded-2xl p-8 sm:p-12 text-center space-y-5 shadow-xl">
              ...
              <div className="space-y-2">
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  المحاضرة مقفلة بالترتيب الأكاديمي
                </h3>

                <p className="text-xs sm:text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
                  يجب عليك أولاً إكمال واجتياز المحاضرة السابقة:{" "}
                  <span className="text-indigo-400 font-bold">
                    "{prevLessonObj?.titleAr}"
                  </span>{" "}
                  لفتح هذه المحاضرة.
                </p>
              </div>
            ...
            <>
              {/* Video Player */}
              <div
                className="bg-[#0f172a] border border-white/5 rounded-2xl overflow-hidden shadow-xl relative select-none"
                onContextMenu={(e) =>
                  e.preventDefault()
                }
              >
                ...
                {selectedLesson.youtubeId ? (
                  <div className="aspect-video w-full relative bg-black overflow-hidden group">
                    ...
                    <iframe
                      ...
                      src={`https://www.youtube.com/embed/${selectedLesson.youtubeId}?modestbranding=1&rel=0&iv_load_policy=3&disablekb=0&playsinline=1&controls=1`}
                      title={
                  ...
                  <div className="aspect-video bg-indigo-950/20 flex flex-col items-center justify-center p-8 text-center gap-3">
                    ...
                    <p className="text-slate-300 font-bold text-sm">
                      لم يتم إدراج فيديو لهذه المحاضرة بعد
                    </p>
                  </div>
                )}
                  ...
                  <div className="px-6 pt-4">
                    ...
                    {!videoCompleted && (
                      <button
                        ...
                        className="mt-3 w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all cursor-pointer"
                      >
                        تأكيد إتمام مشاهدة المحاضرة
                      </button>
                    )}
                  </div>
                )}
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 ltr-dir">
                    ...
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 flex items-center gap-1.5 shrink-0 border ${
                        isCompleted
                ...
                <div className="bg-[#0f172a] border border-white/5 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
                        ...
                        (option, idx) => {
                          ...
                          return (
                            <button
                              ...
                              className={`text-right p-4 rounded-xl border text-xs sm:text-sm transition-all duration-150 flex items-center justify-between gap-4 font-medium ${
                                quizSubmitted
                                  ? "cursor-default"
                                  : "cursor-pointer"
                              } ${
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {quizSuccess ? (
                          <p className="text-emerald-400 text-xs sm:text-sm font-bold flex items-center gap-1">
                            ...
                            <span>
                              إجابة صحيحة وممتازة! أحسنت، تم منحك 15 نقطة.
                            </span>
                          </p>
                        ) : (
                    ...
                    <span className="text-[11px] text-slate-500">
                      سؤال تفاعلي لتأكيد استيعابك للمحاضرة
                    </span>
                  </div>
                </div>
              )}
